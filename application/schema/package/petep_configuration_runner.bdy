create or replace package body petep_configuration_runner is
  ...
  cursor gcur_test_case_instance(p_test_case_id number) is
    select tc_i.plsql_block_id,
           ...
           inpar.value input,
           outpar.value output
      from pete_plsql_block_in_case tc_i, pete_plsql_block blk, pete_input_param inpar, pete_output_param outpar
...
--dbms_output.put_line('debug ' || p_xml.getclobval());    
    petep_assert.eq(a_expected_in => prec_tc_instance.output , a_actual_in => p_xml, a_comment_in =>  'assert vysledku parametricky');
    write_runlog(prec => lrec_runlog);
  ...
  -------------------------------------------------------------------------------------------------------------------------------
  procedure run_test_case(p_id in pete_test_case.id%type) is
    lrec_test_case pete_test_case%rowtype;
  begin
    select * into lrec_test_case from pete_test_case where id = p_id;
  ...
  -------------------------------------------------------------------------------------------------------------------------------
  procedure run_test_case(p_code pete_test_case.code%type) is
    lrec_test_case pete_test_case%rowtype;
  begin
  ...
  -------------------------------------------------------------------------------------------------------------------------------
  procedure run_test_script(prec_test_script pete_test_script%rowtype) is
  ...
  -------------------------------------------------------------------------------------------------------------------------------
  procedure run_test_script(p_id pete_test_script.id%type) is
  ...
  -------------------------------------------------------------------------------------------------------------------------------
  procedure run_test_script(p_code pete_test_script.code%type) is
    lrec_test_script pete_test_script%rowtype;
  begin
    select * into lrec_test_script from pete_test_script where code = p_code;
    run_test_script(prec_test_script => lrec_test_script);
  end run_test_script;
