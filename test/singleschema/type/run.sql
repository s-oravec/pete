@&&run_dir_begin

prompt Install test package UT_PETET_PLSQL_BLOCK
@@ut_petet_plsql_block.pkg

prompt Install test package UT_PETET_INPUT_ARGUMENT
@@ut_petet_input_argument.pkg

prompt Install test package UT_PETET_EXPECTED_RESULT
@@ut_petet_expected_result.pkg

prompt Install test package UT_PETET_PLSQL_BLOCK_IN_CAE
@@ut_petet_plsql_block_in_case.pkg

@&&run_dir_end
