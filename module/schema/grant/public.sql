rem Types
prompt Granting EXECUTE privilege on type PETET_EXPECTED_RESULT to PUBLIC
grant EXECUTE on PETET_EXPECTED_RESULT to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_INPUT_ARGUMENT to PUBLIC
grant EXECUTE on PETET_INPUT_ARGUMENT to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_PLSQL_BLOCK to PUBLIC
grant EXECUTE on PETET_PLSQL_BLOCK to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_PLSQL_BLOCK_IN_CASE to PUBLIC
grant EXECUTE on PETET_PLSQL_BLOCK_IN_CASE to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_PLSQL_BLOCKS_IN_CASE to PUBLIC
grant EXECUTE on PETET_PLSQL_BLOCKS_IN_CASE to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_TEST_CASE to PUBLIC
grant EXECUTE on PETET_TEST_CASE to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_TEST_CASE_IN_SUITE to PUBLIC
grant EXECUTE on PETET_TEST_CASE_IN_SUITE to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_TEST_CASES_IN_SUITE to PUBLIC
grant EXECUTE on PETET_TEST_CASES_IN_SUITE to PUBLIC;

prompt Granting EXECUTE privilege on type PETET_TEST_SUITE to PUBLIC
grant EXECUTE on PETET_TEST_SUITE to PUBLIC;


rem Views
--TODO: ALL/USER/DBA
prompt Granting SELECT privilege on view PETEV_TEST_SUITE to PUBLIC
grant SELECT on PETEV_TEST_SUITE to PUBLIC;

prompt Granting SELECT privilege on view PETEV_TEST_CASE_IN_SUITE to PUBLIC
grant SELECT on PETEV_TEST_CASE_IN_SUITE to PUBLIC;

prompt Granting SELECT privilege on view PETEV_PLSQL_BLOCK_IN_CASE to PUBLIC
grant SELECT on PETEV_PLSQL_BLOCK_IN_CASE to PUBLIC;

prompt Granting SELECT privilege on view PETEV_TEST_CASE to PUBLIC
grant SELECT on PETEV_TEST_CASE to PUBLIC;

prompt Granting SELECT privilege on view PETEV_EXPECTED_RESULT to PUBLIC
grant SELECT on PETEV_EXPECTED_RESULT to PUBLIC;

prompt Granting SELECT privilege on view PETEV_INPUT_ARGUMENT to PUBLIC
grant SELECT on PETEV_INPUT_ARGUMENT to PUBLIC;

prompt Granting SELECT privilege on view PETEV_PLSQL_BLOCK to PUBLIC
